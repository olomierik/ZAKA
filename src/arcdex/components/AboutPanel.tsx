...
function ago(iso: string) {
  ...
  return s < 3600 ? `${Math.floor(s / 60)} min ago` : s < 86400 ? `${Math.floor(s / 3600)} hours ago` : `${Math.floor(s / 86400)} days ago`
}
...
export default function AboutPanel({ address, symbol, info, pool, chain, rows, supply, profiles, navigate }: {
  ...
  const host = (u: string) => { try { return new URL(u).hostname.replace(/^www\./, '') } catch { return 'Website' } }
  ...
  return (
    <div style={{ background: 'var(--adx-card-bg)', border: '1px solid var(--adx-card-border)', borderRadius: 12, marginTop: 16, padding: 14 }}>
      <div style={{ fontWeight: 800, fontSize: '0.9rem' }}>About {symbol}</div>
      ...
      {pool && <Bar left={pool.txns24h.buys} right={pool.txns24h.sells} leftLabel={`${pool.txns24h.buys.toLocaleString()} buys`} rightLabel={`${pool.txns24h.sells.toLocaleString()} sells`} />}
      {rows.length > 0 && <Bar left={buyVol} right={sellVol} leftLabel={`$${fmt(buyVol)} vol.`} rightLabel={`$${fmt(sellVol)} vol.`} />}
      {rows.length > 0 && <Bar left={buyers} right={sellers} leftLabel={`${buyers} buyers`} rightLabel={`${sellers} sellers`} />}
      {rows.length > 0 && <div style={{ fontSize: '0.64rem', color: 'var(--text-muted)', marginTop: -4, marginBottom: 8 }}>Volume and wallets from the last {rows.length} trades.</div>}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
        ...
        {link(`https://x.com/search?q=${encodeURIComponent(`${address} OR $${symbol}`)}`, 'Search on 𝕏')}
      </div>
      {more && (
        <div style={{ marginTop: 12 }}>
          {row('Launchpad', chain?.portal ? `Argus · Portal ${chain.portal}` : pool?.dex === 'argus' ? 'Argus' : pool?.dex ?? '—')}
          {row('Supply', supply ? fmt(supply) : '—')}
          {row('Network', 'Arc')}
          {row('Created', pool?.createdAt ? ago(pool.createdAt) : '—')}
          ...
          {pool && row('Pool', <span style={{ fontFamily: 'var(--mono)' }} title={pool.pool}>{short(pool.pool)}</span>)}
          {chain?.creator && row(chain.creatorLabel, <Who address={chain.creator} profiles={profiles} navigate={navigate} />)}
          {chain?.hook && row('Hook', <a href={`${ARC_EXPLORER}/address/${chain.hook}`} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--adx-accent)', fontFamily: 'var(--mono)' }}>{short(chain.hook)}</a>)}
          {chain?.buyTaxBps != null && row('Creator tax', `${chain.buyTaxBps / 100}% buy · ${(chain.sellTaxBps ?? 0) / 100}% sell`)}
          {row('Bonded', chain?.bonded == null ? '—' : chain.bonded ? 'Yes — graduated' : 'Not yet')}
          {info?.gtScore != null && row('GT score', `${info.gtScore.toFixed(0)} / 100`)}
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 10 }}>
            ...
            {link(`${ARC_EXPLORER}/token/${address}`, 'Explorer')}
          </div>
        </div>
      )}
      <div style={{ textAlign: 'center', marginTop: 10 }}>
        <button onClick={() => setMore(m => !m)} style={{ background: 'var(--bg-2)', border: '1px solid var(--adx-card-border)', color: 'var(--text-muted)', borderRadius: 7, padding: '4px 12px', fontSize: '0.72rem', cursor: 'pointer' }}>{more ? 'View less' : 'View more'}</button>
      </div>
    </div>
  )
}
