    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: 'min(640px, 100%)', background: 'var(--adx-card-bg)', border: '1px solid var(--adx-card-border)', borderRadius: 14, padding: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
          {canNativeShare && (
            <button onClick={() => void navigator.share({ files: [file!], text: `${text} ${card.link}` }).catch(() => {})} style={btn('var(--adx-accent)')}>{T("Share…")}</button>
          )}
          <a href={tweetUrl(text, card.link)} target="_blank" rel="noopener noreferrer" style={{ ...btn('#000'), textDecoration: 'none', border: '1px solid #333' }}>{T("Post on X")}</a>
          <button onClick={download} disabled={!url} style={btn('var(--bg-2)')}>{T("Download image")}</button>
          <button onClick={() => { void navigator.clipboard?.writeText(card.link); setCopied(true) }} style={btn('var(--bg-2)')}>{copied ? T("Link copied ✓") : T("Copy my referral link")}</button>
        </div>
        {referralsLive && (
