...
import {
  ...
  getTraderPositions, getTraderStats, getTrades, socialWrite, triggerIndex,
  type Clan, type IndexedTrade, type Period, type PnlPoint, type PositionRow, type Profile, type Thesis, type TraderStats,
} from '../api/social'
...
export default function TraderPage({ address, navigate }: { address: string; navigate: (p: Page) => void }) {
  ...
  if (!resolved) return (
    <div className="token-page" style={{ padding: 24, color: 'var(--text-muted)' }}>
      {missing ? <>No trader called @{address}. <button className="disc-link" onClick={() => navigate({ name: 'leaderboard' })}>See the leaderboard</button></> : 'Loading…'}
    </div>
  )
  ...
  return (
    <div className="token-page">
      <div className="profile-banner" style={profile?.banner_url && /^https:\/\//.test(profile.banner_url) ? { backgroundImage: `url("${profile.banner_url.replace(/"/g, '')}")` } : undefined}>
        <button className="back-btn" onClick={() => history.length > 1 ? history.back() : navigate({ name: 'leaderboard' })}>← Back</button>
      </div>
      ...
      <div style={{ padding: '0 16px' }}>
        <div style={{ display: 'flex', gap: 16, alignItems: 'flex-end', flexWrap: 'wrap', marginTop: -36 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', paddingBottom: 4 }}>
            <button onClick={() => { void navigator.clipboard?.writeText(profileUrl); setCopied('profile'); setTimeout(() => setCopied(''), 1500) }} style={btn('var(--bg-2)')}>{copied === 'profile' ? 'Copied ✓' : 'Share'}</button>
            ...
            {isMe ? (
              <>
                <button onClick={() => setModal('edit')} style={btn('var(--bg-2)')}>Edit profile</button>
                <button onClick={() => { void navigator.clipboard?.writeText(referralLink(addr, profile)); setCopied('invite'); setTimeout(() => setCopied(''), 1500) }} style={btn('var(--adx-accent)')}>{copied === 'invite' ? 'Link copied ✓' : 'Copy invite link'}</button>
              </>
            ) : (
              <>
                <button onClick={() => me ? setModal('send') : setErr('Connect or unlock a wallet to send cash')} style={btn('var(--bg-2)')}>Send cash</button>
        ...
        <div style={{ marginTop: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 16, marginTop: 10, fontSize: '0.8rem', flexWrap: 'wrap' }}>
            ...
            {allStats?.avg_hold_seconds != null && <span><span style={{ color: 'var(--text-muted)' }}>Avg hold</span> <b>{dur(allStats.avg_hold_seconds)}</b></span>}
            <span><b>{allStats?.trades ?? 0}</b> <span style={{ color: 'var(--text-muted)' }}>trades</span></span>
            {joined && <span><span style={{ color: 'var(--text-muted)' }}>Joined</span> <b>{new Date(joined).toLocaleDateString(undefined, { month: 'short', year: 'numeric' })}</b></span>}
          </div>
          {mutuals.length > 0 && (
            <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 8, fontSize: '0.74rem', color: 'var(--text-muted)' }}>
              <span style={{ display: 'flex' }}>{mutuals.slice(0, 3).map((m, i) => <span key={m.address} style={{ marginLeft: i ? -6 : 0 }}><Avatar address={m.address} url={m.avatar_url} size={18} /></span>)}</span>
              Followed by {mutuals.slice(0, 2).map(mutualName).join(', ')}{mutuals.length > 2 ? ` and ${mutuals.length - 2} other${mutuals.length > 3 ? 's' : ''} you follow` : ''}
            </div>
          )}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ ...card, padding: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, gap: 8, flexWrap: 'wrap' }}>
              <b style={{ fontSize: '0.85rem' }}>PnL</b>
              <div style={{ display: 'flex', gap: 4 }}>
                {PERIODS.map(p => <button key={p.k} onClick={() => setPeriod(p.k)} className={`disc-sub${period === p.k ? ' active' : ''}`}>{p.label}</button>)}
              </div>
            </div>
            <PnlChart points={pnl} sinceMs={sinceMs} />
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 10, marginTop: 12 }}>
              <Stat label={`Realized · ${since.label}`} value={pstats ? signed(pstats.realized_pnl) : '…'} color={(pstats?.realized_pnl ?? 0) >= 0 ? 'var(--green)' : 'var(--red)'} />
              <Stat label={`Volume · ${since.label}`} value={pstats ? money(pstats.volume_usdc) : '…'} />
              <Stat label="Open positions" value={<span className="sensitive">{money(openValue)}</span>} />
              {isMe && <Stat label="Cash" value={<span className="sensitive">{cash == null ? '…' : money(cash)}</span>} />}
            </div>
            {isMe && (
              <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                <button onClick={() => setModal('deposit')} style={{ ...btn('var(--adx-accent)'), flex: 1 }}>Deposit</button>
                <button onClick={() => setModal('withdraw')} style={{ ...btn('var(--bg-2)'), flex: 1 }}>Withdraw</button>
              </div>
            )}
          </div>
          ...
          <div style={card}>
            <div style={head}>
              <div style={{ display: 'flex', gap: 4 }}>
                {(['open', 'closed'] as const).map(t => <button key={t} onClick={() => setPosTab(t)} className={`disc-sub${posTab === t ? ' active' : ''}`}>{t === 'open' ? `Open (${open.length})` : 'Closed'}</button>)}
              </div>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', fontSize: '0.72rem', fontWeight: 500 }}>
                {posTab === 'open' && <label style={{ display: 'flex', gap: 4, alignItems: 'center', color: 'var(--text-muted)', cursor: 'pointer' }}><input type="checkbox" checked={dust} onChange={e => setDust(e.target.checked)} /> Show dust</label>}
                <select value={sort} onChange={e => setSort(e.target.value as typeof sort)} className="disc-select">
                  <option value="recent">Recent</option><option value="value">Value</option><option value="pnl">PnL</option>
                </select>
              </div>
            </div>
            {loading ? <Empty>Loading…</Empty> : shownPos.length === 0 ? <Empty>{posTab === 'open' ? (isMe ? 'No open positions yet — buy a coin to see it here.' : 'No open positions traded through ARCDEX.') : 'No closed positions yet.'}</Empty> : (
                      ...
                      <tr key={p.token} onClick={() => openToken(p.token, p.meta)} style={{ borderBottom: '1px solid var(--adx-card-border)', cursor: 'pointer' }}>
                        <td style={{ padding: '10px 16px' }}>
                          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                            ...
                            <div><b>${p.meta?.symbol ?? shortAddr(p.token)}</b><div style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>invested <span className="sensitive">{money(p.bought_usdc)}</span> · {ago(p.last_trade)}</div></div>
                          </div>
                        </td>
          ...
          <div style={card}>
            <div style={head}>
              <span>Swaps</span>
              <div style={{ display: 'flex', gap: 4 }}>
                {(['all', 'buy', 'sell'] as const).map(t => <button key={t} onClick={() => setSwapTab(t)} className={`disc-sub${swapTab === t ? ' active' : ''}`}>{t === 'all' ? 'All swaps' : t === 'buy' ? 'Buys' : 'Sells'}</button>)}
              </div>
            </div>
            {swaps.length === 0 ? <Empty>No {swapTab === 'all' ? 'trades' : swapTab + 's'} through ARCDEX yet.</Empty> : (
                  ...
                  {swaps.map(t => {
                    ...
                    return (
                      <tr key={t.tx_hash + t.log_index} style={{ borderBottom: '1px solid var(--adx-card-border)' }}>
                        <td style={{ padding: '8px 16px', color: t.side === 'buy' ? 'var(--green)' : 'var(--red)', fontWeight: 700 }}>{t.side === 'buy' ? 'Buy' : 'Sell'}</td>
        ...
        <div className="token-detail-swap">
          {pinned && pinned.likes > 0 && (
            <div style={{ ...card, border: '1px solid rgba(250,204,21,0.35)' }}>
              <div style={head}><span>📌 Pinned thesis</span><span style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 500 }}>♥ {pinned.likes}</span></div>
              <ThesisItem t={pinned} meta={meta} openToken={openToken} />
            </div>
          )}
          <div style={card}>
            <div style={head}>Theses <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 500 }}>{theses.length}</span></div>
            {theses.length === 0 ? <Empty>No theses yet.</Empty> : theses.map(t => <ThesisItem key={t.id} t={t} meta={meta} openToken={openToken} />)}
          </div>
        </div>
      </div>
...
function ThesisItem({ t, meta, openToken }: { t: Thesis; meta: Map<string, TokenMeta>; openToken: (token: string, m?: TokenMeta) => void }) {
  ...
  return (
    <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--adx-card-border)' }}>
      ...
      {t.position_usd != null && <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', marginTop: 4 }}>Position when posted: {money(t.position_usd)}</div>}
    </div>
  )
}
