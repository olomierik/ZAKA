...
export default function ClanPage({ slug, navigate }: { slug: string; navigate: (p: Page) => void }) {
  ...
  if (clan === undefined) return <div className="loading-state">Loading…</div>
  if (clan === null) return <div className="token-page"><button className="back-btn" onClick={() => navigate({ name: 'clans' })}>← Clans</button><div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>This clan doesn't exist (or the clans database isn't set up yet).</div></div>

  return (
    <div className="token-page">
      <button className="back-btn" onClick={() => navigate({ name: 'clans' })}>← Clans</button>
      <div style={{ ...card, overflow: 'hidden', marginTop: 8 }}>
        ...
        <div style={{ padding: '0 18px 16px', display: 'flex', gap: 14, alignItems: 'flex-end', flexWrap: 'wrap', marginTop: -30 }}>
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            ...
            <div style={{ fontSize: '0.74rem', color: 'var(--text-muted)', marginTop: 4 }}>👥 {members.length} member{members.length === 1 ? '' : 's'} · {members.reduce((s, m) => s + (m.volume_usdc > 0 ? 1 : 0), 0)} active</div>
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button className="btn-ghost" onClick={() => { void navigator.clipboard?.writeText(`https://arcdex.online/clans/${clan.slug}`); setCopied(true) }}>{copied ? 'Link copied ✓' : '⤴ Share'}</button>
            {isOwner && <button className="btn-ghost" onClick={() => setEditing(true)}>Edit</button>}
            {me && (inThis
              ? <button className="btn-ghost" disabled={busy} onClick={() => void act('clan.leave')}>Leave clan</button>
              : <button className="btn-primary" style={{ padding: '8px 16px' }} disabled={busy || !!myClan} title={myClan ? 'Leave your current clan first' : ''} onClick={() => void act('clan.join')}>{myClan ? 'In another clan' : 'Join clan'}</button>)}
          </div>
        </div>
      ...
      <div className="token-detail-grid" style={{ padding: 0, marginTop: 16 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ ...card, padding: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Clan profit</span>
          ...
          <div style={{ ...card, marginTop: 16, overflow: 'hidden' }}>
            <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--adx-card-border)', fontWeight: 700, fontSize: '0.86rem' }}>Clan holdings <span style={{ color: 'var(--text-muted)', fontWeight: 500 }}>{priced.length}</span></div>
            {priced.length === 0 ? <div style={{ padding: 24, textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.82rem' }}>No open positions traded through ARCDEX yet.</div> : (
              <div style={{ overflowX: 'auto' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.8rem', minWidth: 520 }}>
                  <thead><tr>{['Token', 'Members', 'Combined position', 'Combined PnL'].map((h, i) => <th key={h} style={{ padding: '8px 16px', textAlign: i >= 2 ? 'right' : 'left', fontSize: '0.7rem', color: 'var(--text-muted)', fontWeight: 600 }}>{h}</th>)}</tr></thead>
                  <tbody>{priced.map(p => (
        ...
        <div className="token-detail-swap">
          <div style={{ ...card, overflow: 'hidden' }}>
            ...
            {tab === 'members' && (
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 16px', fontSize: '0.76rem', color: 'var(--text-muted)' }}>
                  ...
                  {me && <button className="disc-link" disabled={busy} onClick={() => void followAll()}>Follow all</button>}
                </div>
                {members.map(m => {
            ...
            {tab === 'feed' && (trades.length === 0 ? <div style={{ padding: 20, textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.8rem' }}>No member trades yet.</div> : trades.map(t => {
              ...
              return (
                <div key={t.tx_hash + t.log_index} className="rail-row" style={{ padding: '8px 16px', fontSize: '0.78rem', gap: 6 }}>
                  ...
                  <span style={{ color: t.side === 'buy' ? 'var(--green)' : 'var(--red)', fontWeight: 700 }}>{t.side === 'buy' ? 'bought' : 'sold'}</span>
            ...
            {tab === 'thesis' && (theses.length === 0 ? <div style={{ padding: 20, textAlign: 'center', color: 'var(--text-muted)', fontSize: '0.8rem' }}>No theses from members yet.</div> : theses.map(t => (
              <div key={t.id} style={{ padding: '10px 16px', borderTop: '1px solid var(--adx-card-border)', fontSize: '0.8rem' }}>
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}><Avatar address={t.author} url={profiles.get(t.author)?.avatar_url} size={20} /><b>{profiles.get(t.author)?.username ? `@${profiles.get(t.author)!.username}` : shortAddr(t.author)}</b> on <button className="disc-link" onClick={() => navigate({ name: 'argus', address: t.token, pool: meta.get(t.token)?.pool ?? '' })}>${meta.get(t.token)?.symbol ?? shortAddr(t.token)}</button></div>
